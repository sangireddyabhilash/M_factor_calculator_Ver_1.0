...
def success(name):
   ...
   tempo1 = name
   tempo2 = name
   os.system("runenergyplus ./static/Data/"+tempo+"input/"+tempo+".idf /usr/local/EnergyPlus-8-1-0/WeatherData/epw/delhi.epw")
   ...
   while (float(tempo) > 0.05):
      os.system("runenergyplus ./static/Data/"+tempo+"/"+tempo+".idf /usr/local/EnergyPlus-8-1-0/WeatherData/epw/delhi.epw")
...
def poc():
   global odepth=0.05
   global odepth = global odepth + 0.05
   if odepth<=0.95:
